"""Resumable sharded inference for frozen policy v4all_cap300_lgbm_m2.

Same sharding, manifest, resume, and merge guarantees as run_infer_v3b.py (whose
generic helpers are reused); scoring uses frozen_v4 retrieval + 32 features.

  python3 run_infer_v4.py --split test --workers 4 --out-dir artifacts/submissions/v4all_m2_v1
"""
...
REPO = SRC.parents[2]
DEFAULT_MODEL = REPO / "artifacts" / "official_index" / "matcher_v4all_m2.pkl"
MODEL_SHA256 = "5f065bda9bd1a11afbf9e3c9f6ae7a455381f44b83a664e7b69075bd0779cdef"
MODEL_KEY = "M2_allneg_extra_fit15k"
...
def load_booster(model_path: Path):
    ...
    if bundle.get("variant") != "v4all" or int(bundle.get("cap")) != P.CAP or bundle.get("routes") != P.ROUTES:
        raise RuntimeError("bundle retrieval policy differs from frozen_v4")
    res = bundle["results"][MODEL_KEY]
    if res["features"] != P.FEATURE_NAMES:
        raise RuntimeError("bundle feature order differs from frozen_v4")
    if abs(res["threshold"] - P.THRESHOLD) > 1e-12:
        raise RuntimeError(f"bundle threshold {res['threshold']} != {P.THRESHOLD}")
    return bundle["models"][MODEL_KEY].booster_


def _init_worker(index_path, pair_path, aux_path, model_path, bounds, pair_bounds, aux_bounds, n_targets):
    conns = []
    for pth in (index_path, pair_path, aux_path):
        c = sqlite3.connect(f"file:{pth}?mode=ro", uri=True, check_same_thread=False)
        c.execute("PRAGMA cache_size=-196608")
        conns.append(c)
    _W["db"] = conns[0]
    _W["ret"] = P.Retriever(conns[0], conns[1], conns[2], n_targets, bounds, pair_bounds, aux_bounds)
...
def score_rows(rows, batch: int = 256):
    ...
    for i in range(0, len(rows), batch):
        ...
        for r, (c, sc, rt) in zip(chunk, got):
            if c:
                mats.append(P.block_features(P.s1_view(r), c, sc, rt, recs))
            sizes.append(len(c))
        ...
        for r, (c, _, _), n in zip(chunk, got, sizes):
            pr = probs[k : k + n]
            k += n
            out.append((r["entity_id"], c, [t for t, p in zip(c, pr) if p >= P.THRESHOLD]))
    return out
...
def main():
    ...
    for name, st in (("pair", pair_stats), ("aux", aux_stats)):
        if st.get("split") != args.split:
            raise SystemExit(f"{name} index split mismatch")
    if not aux_stats.get("complete") or (args.split == "test" and not pair_stats.get("complete")):
    ...
    if args.split == "test":
        expected = sum(sum(1 for _ in iter_tsv(args.dataset_root / "test" / f"test_source{s}.tsv", SOURCE_H)) for s in (2, 3))
        if n_targets != expected:
            raise SystemExit(f"test index has {n_targets} targets, expected {expected}")
    if pair_stats["targets"] != n_targets or aux_stats["targets"] != n_targets:
        raise SystemExit("target universe mismatch between indexes")
    ...
    config = {
        ...
        "base_policy_code_sha256": sha256_file(SRC / "frozen_v3b" / "policy.py"),
        "writer_code_sha256": sha256_file(Path(__file__)),
        "model_sha256": model_sha, "model_key": MODEL_KEY,
        "cap": P.CAP, "threshold": P.THRESHOLD, "feature_names": P.FEATURE_NAMES,
        "split": args.split, "s1_sha256": mf[s1_rel]["sha256"],
        "index": str(index), "index_bytes": index.stat().st_size,
        "pair_stats": pair_stats, "aux_stats": aux_stats, "n_targets": n_targets,
    ...
    ctx = (str(index), str(pair), str(aux), str(args.model), bounds, pair_bounds, aux_bounds, n_targets)

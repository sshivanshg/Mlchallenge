"""Resumable sharded inference for frozen policy v5 (v4all retrieval + selected matcher).

The model key, feature set, and decision rule are read from frozen_v5/selected.json,
written once from selection-fold results. Sharding, manifests, resume, and merge are
identical to run_infer_v3b.py / run_infer_v4.py.

  python3 run_infer_v5.py --split test --workers 4 --out-dir artifacts/submissions/v5_v1
"""
...
REPO = SRC.parents[2]
SELECTED = json.loads((SRC / "frozen_v5" / "selected.json").read_text())
...
def load_booster(model_path: Path):
    ...
    if bundle.get("policy") != "v4all_cap300_lgbm_m2_thr0.70":
        raise RuntimeError("bundle was not trained on frozen v4 retrieval")
    feats = bundle["base_features"] + (bundle["extra2_features"] if N_FEATURES == 43 else [])
    expected = P.FEATURE_NAMES if N_FEATURES == 43 else P.V4_FEATURES
    if feats != expected:
        raise RuntimeError("bundle feature order differs from frozen_v5")
    model = bundle[MODEL_KEY]
    if model.n_features_in_ != N_FEATURES:
        raise RuntimeError("model feature count mismatch")
...
def score_rows(rows, batch: int = 256):
    ...
    for i in range(0, len(rows), batch):
        ...
        for r, (c, sc, rt) in zip(chunk, got):
            if c:
                a = P.s1_view(r)
                mats.append(P.features(a, c, sc, rt, recs) if N_FEATURES == 43 else P.block_features(a, c, sc, rt, recs))
            sizes.append(len(c))
...
def main():
    ...
    config = {
        ...
        "v4_policy_code_sha256": sha256_file(SRC / "frozen_v4" / "policy.py"),
        ...
        "feature_names": P.FEATURE_NAMES if N_FEATURES == 43 else P.V4_FEATURES,
